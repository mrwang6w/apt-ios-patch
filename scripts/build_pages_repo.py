...
PATCHED_PACKAGES = [
    ...
    },
    {
        ...
        "source": PATCHED_DIR / "560_CTW_Pro(无根版)_5.6.0-offline7_com.amg456.CTWPro.rootless560_deep_offline_ustar.deb",
        "deb_name": "com.amg456.CTWPro.rootless560_5.6.0-offline7_deep_offline_ustar.deb",
        "publish_name": "560_CTW Pro(无根版) 5.6.0-offline7 Deep Offline USTAR",
        "publish_section": "CTW Pro",
        "publish_desc": "授权测试补丁包：禁用 CTW.dylib 卡密激活、启动复核、周期心跳、失败弹窗和退出路径；本地 fix.dylib 精确覆盖 MachinePreferences 的 randomPreferences: action，在 App runtime 缺少 LKDeviceConfig 时从 rootless 路径显式加载已补丁 CTW.dylib，并按 config、缓存 JSON、defaultConfig 顺序选择硬件/固件基线，再生成身份字段并调用原 performeMachineStub，不启动 /vd；失败时显示精确阶段码；保留原 38 项 entitlements。",
