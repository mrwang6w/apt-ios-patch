...
`0CTW.dylib` 和 `ctwsup.dylib` 均未修改；`offline5` 只在运行时调用经过偏移校验的
原 stub。
...
`offline5` 已从固定原包连续完成两次全量重建；deb、`fix.dylib`、主程序和
`CTW.dylib` 的 SHA256 均逐项一致。

## 7. 最终产物

```text
patched/560_CTW_Pro(无根版)_5.6.0-offline5_com.amg456.CTWPro.rootless560_deep_offline_ustar.deb
```

- Package: `com.amg456.CTWPro.rootless560`
- Version: `5.6.0-offline5`
- Size: `27,002,844` bytes
- SHA256: `ddd484af44db1afd02d45462dcd8e5b160a235ac30c5eee7839fa466105dfd50`

最终 Mach-O：

- `CTW Pro`: `3fbe426038ce7874a33438563eb804a7c1652d65d614cb96944522f841e003f5`
- `fix.dylib`: `d57e0346098d5a2423b0f5942e5a8130ee001c9c98e9717ba43f16c43773c18f`
...
返回 502 后显示“超时或非法请求”。`offline5` 的 NIB action、运行时注册点、原 apply
入口、本地 helper、缓存写入 selector 和载荷均已完成静态闭环。

当前设备可以通过 USB 枚举并正常桌面启动 App，但 Frida `17.15.3` 和与历史服务端
匹配的 `17.11.0` 客户端附加均返回 `unexpected early end-of-stream`；SSH/端口转发也
没有可用 root shell。因此本轮无法把新 deb 安装到设备，尚未取得 `offline5` 点击后
“零 `/vd`、缓存身份字段变化、原 stub 执行且不闪退”的最终真机证据。静态构建通过
不替代这项回归，设备端服务恢复后仍需执行该门禁。
...
Package: com.amg456.CTWPro.rootless560
Version: 5.6.0-offline5
Filename: ./debs/com.amg456.CTWPro.rootless560_5.6.0-offline5_deep_offline_ustar.deb
Size: 27002844
SHA256: ddd484af44db1afd02d45462dcd8e5b160a235ac30c5eee7839fa466105dfd50
